...
function Filter({project}) {
  ...
  return (
    <div>
      <p className="font-bold text-4xl p-5 mt-[100px]" ref={project}>Projects</p>
      <div className="grid gap-5 p-2   mt-5 md:grid-cols-2 md:p-5 lg:grid-cols-3 lg:p-5 ">
        {Data.map((item) => (
          <div>
            <div className=" relative group  transition-all duration-500 cursor-pointer  ">
              <img
                src={item.Image}
                alt="products"
              ...
              <h1 className="absolute w-full h-full hover:bg-black hover:bg-opacity-40 top-0 left-0 transition-all duration-75">
                <h1 className="text-lg text-center mt-3 font-extrabold text-[#fff] opacity-0 group-hover:opacity-100">{item.skills}</h1>
